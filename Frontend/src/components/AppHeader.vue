  ...
  <header class="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-lg border-b border-gray-200 shadow-sm ">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ">
      <div class="flex items-center justify-between h-20">
        ...
        <!-- Logo et nom du site -->
        <div class="flex items-center cursor-pointer group hover:scale-105 transition-all"
        ...
        <nav class="hidden md:flex items-center gap-1">
          <a v-for="item in menuItems" :key="item.id" @click="$emit('navigate', item.id)" :class="['px-4 py-2 rounded-lg font-medium text-sm transition-all cursor-pointer',
            currentPage === item.id
        ...
        <div class="hidden md:flex items-center gap-3">
          ...
          <template v-if="isLoggedIn">
            ...
            <button @click="$emit('navigate', 'generator')"
              class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 py-2.5 rounded-lg">
              Créer un QCM
            </button>
          </template>
          ...
          <!-- Si non connecté -->
          <template v-else>
            <button @click="$emit('navigate', 'login')" class="text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-100">
              Se connecter
            </button>
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-2"
      ...
      <div v-if="mobileMenuOpen" class="md:hidden border-t border-gray-200 bg-white">
        <div class="px-4 py-4 space-y-2">
          <a v-for="item in menuItems" :key="item.id" @click="handleMobileNavigation(item.id)" :class="[
          ...
          <button @click="handleMobileNavigation('generator')"
